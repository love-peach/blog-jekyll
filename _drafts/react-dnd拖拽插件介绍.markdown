---
layout: post
title: "react-dnd拖拽插件介绍"
date: 2016-03-14 19:08
...
**DragSource(type, spec, collect)　和　DropTarget(type, spec, collect)**

- type 必须。type 是自定义的，可以是 string，symbol，也可以是用一个函数来返回该组件的其他 props。该组件只能放到相同 type 的 DropTarget 中。
- spec 必须。一个带有特定方法的纯 Object，里面是一些响应拖拽事件的方法。(往下翻)
...
**DragSource#collect(connect, monitor) 和　DropTarget#collect(connect, monitor)**
...
转载链接：　[React-DnD 的使用](https://segmentfault.com/a/1190000004006185)